...
class AttendanceRepositoryImpl implements AttendanceRepository {
  ...
  Future<void> saveAttendance(
    ...
    if (workerList.isNotEmpty) {
      ...
      final netAmount =
          (worker.dailyWage * wageFactor) + overtimeAmount + bonus - deduction;

      final existingWage = await _client
          .from('daily_wages')
          .select('id')
          .eq('attendance_id', attendanceWithContractor.id)
          .maybeSingle();
      final dailyWageId = existingWage != null
          ? existingWage['id'] as String
          : const Uuid().v4();
...
final attendanceStreamProvider =
    StateNotifierProvider.family<AttendanceNotifier, List<Attendance>, String?>(
      (ref, projectId) {
...
final workerAttendanceStreamProvider =
